...
trajectories_U = zeros(numPoints,2,T_sim);  % Preallocate trajectories_U array
...
for i = 1:numPoints
    ...
    plot(squeeze(trajectories_U(i,1,:)), 'm', 'LineWidth', 1.5);
    xlabel('Time');
    ylabel(sprintf('X Coordinate (Point %d)', i));
    title(sprintf('Trajectories Point %d', i));
    legend('Estimated', 'Real', 'GPS', 'Control Noise');
end
...
for i = 1:numPoints
    ...
    plot(squeeze(trajectories_U(i,2,:)), 'm', 'LineWidth', 1.5);
    xlabel('Time');
